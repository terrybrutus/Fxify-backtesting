...
type TvAlert = {
  ...
  staleEntryAlertAllowed?: boolean;
  actionableAlertAllowed?: boolean;
  alertCooldownSeconds?: number;
  alertCooldownReady?: boolean;
  snapback?: boolean;
  pushThrough?: boolean;
  ...
  reclaimDistanceRatio?: number;
  pushThroughLevel?: number;
  pushThroughDistance?: number;
  pushThroughDistanceRatio?: number;
...
const LATEST_PLAYBOOK_VERSION = "raw-parity-v21";
...
function normalizeAlertPayload(
  ...
  return {
    ...
    alertCoverage:
      typeof item.alertCoverage === "string" ? item.alertCoverage : undefined,
    rawSignal:
    ...
    sameBarLiveExit:
      typeof item.sameBarLiveExit === "boolean"
        ? item.sameBarLiveExit
        : undefined,
    rawLongSignal:
    ...
    enterAlertAllowed:
      typeof item.enterAlertAllowed === "boolean"
        ? item.enterAlertAllowed
        : undefined,
    dnhAlertAllowed:
      typeof item.dnhAlertAllowed === "boolean"
        ? item.dnhAlertAllowed
        : undefined,
    staleEntryAlertAllowed:
      typeof item.staleEntryAlertAllowed === "boolean"
        ? item.staleEntryAlertAllowed
        : undefined,
    actionableAlertAllowed:
      typeof item.actionableAlertAllowed === "boolean"
        ? item.actionableAlertAllowed
        : undefined,
    alertCooldownSeconds: asNumber(item.alertCooldownSeconds),
    alertCooldownReady:
      typeof item.alertCooldownReady === "boolean"
        ? item.alertCooldownReady
        : undefined,
    snapback:
    ...
    reclaimDistance: asNumber(item.reclaimDistance),
    reclaimDistanceRatio: asNumber(item.reclaimDistanceRatio),
    pushThroughLevel: asNumber(item.pushThroughLevel),
    pushThroughDistance: asNumber(item.pushThroughDistance),
    pushThroughDistanceRatio: asNumber(item.pushThroughDistanceRatio),
...
function scoreTouch(
  ...
  if (blockers.some((blocker) => blocker.includes("Next 1m kept pushing"))) {
    ...
    doNow = `${plainTradeWord(touch.direction)} NOW. Use the listed entry, stop, and target. Skip if the move has already run.`;
...
useSessionFilter = input.bool(false, title="Block Outside Active Session")
activeSession = input.session("0300-1200", title="Active Session To Measure")
...
// Raw Brutus signal layer. Pure touch drives live decisions; ORIG proves parity with the old triangle indicator.
pureLongTouchCondition = lowerSrc <= lower
pureShortTouchCondition = upperSrc >= upper
pureTouchSignal = pureLongTouchCondition or pureShortTouchCondition
// These two conditions intentionally match the original indicator's triangle logic.
...
varip bool alertedLongEnterThisBar = false
varip bool alertedShortEnterThisBar = false
varip bool alertedLongDnhThisBar = false
varip bool alertedShortDnhThisBar = false
varip string lastLongAlertAction = ""
varip string lastShortAlertAction = ""
varip int lastDecisionAlertTimeMs = na
if na(latchedBarTime) or time != latchedBarTime
    ...
    alertedLongEnterThisBar := false
    alertedShortEnterThisBar := false
    alertedLongDnhThisBar := false
    alertedShortDnhThisBar := false
    lastLongAlertAction := ""
    lastShortAlertAction := ""
newLongTouch = pureLongTouchCondition and not rawLongLatched
newShortTouch = pureShortTouchCondition and not rawShortLatched
if newLongTouch
    longTouchProgressPct := barProgressPct
if newShortTouch
    shortTouchProgressPct := barProgressPct
if pureLongTouchCondition
    rawLongLatched := true
if pureShortTouchCondition
    rawShortLatched := true

rawLongSignal = signalMode == "First touch" ? (barstate.isrealtime ? rawLongLatched : pureLongTouchCondition) : rawLongCondition
rawShortSignal = signalMode == "First touch" ? (barstate.isrealtime ? rawShortLatched : pureShortTouchCondition) : rawShortCondition
...
reclaimLevel = direction == "long" ? lower : direction == "short" ? upper : na
pushThroughLevel = direction == "long" ? lower - bandWidth * 0.05 : direction == "short" ? upper + bandWidth * 0.05 : na
reclaimDistance = direction == "long" ? math.max(0.0, close - lower) : direction == "short" ? math.max(0.0, upper - close) : na
pushThroughDistance = direction == "long" ? math.max(0.0, lower - close) : direction == "short" ? math.max(0.0, close - upper) : na
reclaimDistanceRatio = na(reclaimDistance) ? na : reclaimDistance / bandWidth
pushThroughDistanceRatio = na(pushThroughDistance) ? na : pushThroughDistance / bandWidth
...
tp2Text = na(tp2) ? "n/a" : str.tostring(tp2)
tp3Text = na(tp3) ? "n/a" : str.tostring(tp3)
tp4Text = na(tp4) ? "n/a" : str.tostring(tp4)
reclaimText = na(reclaimLevel) ? "n/a" : str.tostring(reclaimLevel)
pushThroughText = na(pushThroughLevel) ? "n/a" : str.tostring(pushThroughLevel)
...
liveAction = overlappingActiveTrade ? "WAIT" : action
liveReason = overlappingActiveTrade ? "New Brutus touch fired while an earlier playbook setup is still active." : reason
livePlainAction = overlappingActiveTrade ? "WAIT. Existing setup still active. Do not open a second trade." : plainAction
alertEntryJson = liveAction == "ENTER" ? entryJson : "null"
alertStopJson = liveAction == "ENTER" ? stopJson : "null"
alertTp1Json = liveAction == "ENTER" ? tp1Json : "null"
alertTp2Json = liveAction == "ENTER" ? tp2Json : "null"
alertTp3Json = liveAction == "ENTER" ? tp3Json : "null"
alertTp4Json = liveAction == "ENTER" ? tp4Json : "null"
...
meaningfulActionChange = action == "ENTER" or action == "DO_NOT_HOLD" or tp1AlreadyTouched
...
actionableAlert = liveAction == "ENTER" or liveAction == "DO_NOT_HOLD" or tp1AlreadyTouched
...
staleEntryAlertAllowed = tp1AlreadyTouched and ((direction == "long" and not alertedLongThisBar) or (direction == "short" and not alertedShortThisBar))
actionableAlertAllowed = (liveAction == "ENTER" and enterAlertAllowed) or (liveAction == "DO_NOT_HOLD" and dnhAlertAllowed) or staleEntryAlertAllowed
...
fullEvidenceAlerts = alertCoverage == "Full evidence"
researchAlert = fullEvidenceAlerts and (firstTouchNewSide or firstTouchOriginalTriangle or confirmedCloseEvent)
shouldAlert = modeReady and alertCooldownReady and (not liveAlertsOnly or barstate.isrealtime) and ((actionableAlert and actionableAlertAllowed and (firstTouchNewSide or firstTouchDecisionChanged or confirmedCloseEvent)) or researchAlert)
...
if showAuditPanel and barstate.islast
    table.cell(auditPanel, 0, 0, "Brutus Playbook raw-parity-v21", text_color=color.white, bgcolor=color.new(color.black, 0))
...
function compactLevel(value?: number) {
  if (value == null || !Number.isFinite(value)) return "?";
  return value.toFixed(2);
}

function compactBandRatio(value?: number) {
  if (value == null || !Number.isFinite(value)) return "?";
  return `${value >= 0 ? "+" : ""}${value.toFixed(3)}bw`;
}
...
function alertGateSummary(alert: TvAlert) {
  ...
  const timingOk =
    ...
    (alert.minBarProgressPct != null && alert.barProgressPct != null
      ...
      : undefined);
  return `${source} | pure ${boolWord(pureTouch)} | original ${boolWord(original)} | live ${boolWord(liveTouch)} | session ${boolWord(alert.inSession)} | timing ${boolWord(timingOk)} (${progress}, ${touchProgress}, ${afterTouch}) | reclaim line ${compactLevel(alert.reclaimLevel)} (${compactBandRatio(alert.reclaimDistanceRatio)}) | push line ${compactLevel(alert.pushThroughLevel)} (${compactBandRatio(alert.pushThroughDistanceRatio)}) | snapback ${boolWord(snapback)} | push-through ${boolWord(push)}`;
}
...
function denialReasonFor(item: AlertDecisionMatch) {
  ...
  if (alert.overlappingActiveTrade === true) {
    return {
      key: "active-setup",
      label: "Denied by active setup",
      plainMeaning:
        "A new Brutus touch fired while a previous playbook setup was still active.",
      action:
...
function strategyDiagnosisFor(item: AlertDecisionMatch) {
  ...
  if (alert.overlappingActiveTrade === true) {
    return {
      key: "overlapping-active-setup",
      family: "Overlapping active setup",
      paperUse: "review" as const,
      plainFinding:
        "Another Brutus touch appeared before the prior setup resolved.",
      paperRule:
        "Do not open a second live trade from this alert. First judge whether the active setup should be held, stopped, or exited.",
      entryPlan: "No fresh entry from this alert while the earlier setup is active.",
      exitPlan:
        "Use the original setup's stop and targets until that setup resolves.",
