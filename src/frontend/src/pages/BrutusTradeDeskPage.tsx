...
plainAction = action == "ENTER" ? tradeWord + " NOW | Entry " + entryText + " | Stop " + stopText + " | TP1 " + tp1Text + " | TP2 " + tp2Text + " | TP3 " + tp3Text + " | TP4 " + tp4Text + " | Check " + checkpointText + " | Reclaim " + reclaimText : action == "WAIT" ? "WAIT. No trade yet. At " + checkpointText + ", price has not reclaimed " + reclaimText + "." : action == "DO_NOT_HOLD" ? "DO NOT ENTER. At " + checkpointText + ", price is still past push-through " + pushThroughText + "." : "SKIP. No trade."
...
if showAuditPanel and barstate.islast
    table.cell(auditPanel, 0, 0, "Brutus Playbook raw-parity-v17", text_color=color.white, bgcolor=color.new(color.black, 0))
...
function alertGateSummary(alert: TvAlert) {
  ...
  const timingOk =
    ...
    (alert.minBarProgressPct != null && alert.barProgressPct != null
      ...
      : undefined);
  return `${source} | pure ${boolWord(pureTouch)} | original ${boolWord(original)} | live ${boolWord(liveTouch)} | session ${boolWord(alert.inSession)} | timing ${boolWord(timingOk)} (${progress}, ${touchProgress}, ${afterTouch}) | reclaim ${compactLevel(alert.reclaimLevel)} | push-through line ${compactLevel(alert.pushThroughLevel)} | snapback ${boolWord(snapback)} | push-through ${boolWord(push)}`;
}
