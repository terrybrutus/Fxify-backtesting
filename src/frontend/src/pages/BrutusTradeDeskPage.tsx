...
waitReason = not notTooEarly ? "Band touch fired, but the candle has not reached the allowed live-decision window yet." : not snapbackOk ? "Band touch fired, but price has not reclaimed enough yet." : "Band touch fired, but the playbook still says wait."
...
reason = signalConflict ? "Both long and short band touches are active. Skip because direction is unclear." : action == "ENTER" ? "Band touch fired and price reclaimed enough for an entry plan." : action == "WAIT" ? waitReason : action == "DO_NOT_HOLD" ? "Band touch fired, but price is still pushing through the band." : skipReason
plainAction = action == "ENTER" ? tradeWord + " NOW | Entry " + entryText + " | Stop " + stopText + " | TP1 " + tp1Text + " | TP2 " + tp2Text + " | TP3 " + tp3Text + " | TP4 " + tp4Text : action == "WAIT" ? "WAIT. No trade yet. Price touched the band but has not reclaimed enough." : action == "DO_NOT_HOLD" ? "DO NOT ENTER. Price is still pushing through the band." : "SKIP. No trade."
...
if showAuditPanel and barstate.islast
    table.cell(auditPanel, 0, 0, "Brutus Playbook raw-parity-v16", text_color=color.white, bgcolor=color.new(color.black, 0))
