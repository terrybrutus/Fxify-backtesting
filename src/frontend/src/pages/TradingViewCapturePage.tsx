...
function normalizePayload(raw: unknown): TvAlert {
  ...
  return {
    ...
    alertCoverage: asString(item.alertCoverage),
    rawSignal:
    ...
    tooEarly: asBoolean(item.tooEarly),
...
function bandRatioLabel(value?: number) {
                          ...
                          {isLatestPlaybookAlert(alert) && (
                            <span className="block max-w-56 whitespace-normal text-muted-foreground">
                              ...
                              {alert.reclaimLevel != null
                                ? `, reclaim ${alert.reclaimLevel.toFixed(2)} (${bandRatioLabel(alert.reclaimDistanceRatio)})`
                                : ""}
                              {alert.pushThroughLevel != null
                                ? `, push line ${alert.pushThroughLevel.toFixed(2)} (${bandRatioLabel(alert.pushThroughDistanceRatio)})`
                                : ""}
                              , snap{" "}
                              {gateLabel(
                                alert.longSnapback ??
